...
We've already seen a few python functions such as print and input, but how we're going to dive into writing
our own functions. To get started, we'll write a function that takes in a number and squares it:
...
One thing made possible by functional programming is the idea of a decorator, which is a higher-order-function that can modify another function. For example, let's write a decorator that announces when a function is about to begin, and when it ends. We can applu this decorator using an @ symbol.
